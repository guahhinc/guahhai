...
$outputFile = Join-Path $basePath "memory.json"
...
if (Test-Path $dataDir) {
    if ((Get-ChildItem $dataDir -Filter "*.jsonl").Count -gt 0) {
...
if (Test-Path $data2ConvertedDir) {
    if ((Get-ChildItem $data2ConvertedDir -Filter "*.jsonl").Count -gt 0) {
...
Write-Host "  Serializing $($allObjects.Count) entries to JSON..."
# Convert to JSON string
$jsonString = $allObjects | ConvertTo-Json -Depth 2 -Compress

# Create JSON file
Set-Content -Path $outputFile -Value $jsonString -Encoding UTF8

$sizeMB = ((Get-Item $outputFile).Length / 1MB).ToString("N2")
Write-Host "Success! Created memory.json ($sizeMB MB)" -ForegroundColor Green
